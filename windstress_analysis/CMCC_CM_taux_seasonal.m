function [ jet_f, jet_xi, mean_jet, std_jet, jet_trends_yrs, jet_loc_f, jet_loc_xi, mean_jet_loc, std_jet_loc, jet_loc_trends_yrs, time_year, jet_DJF, lat_jet_DJF ] = CMCC_CM_taux_seasonal
...
% %% Import Data
% current_path = pwd;
% pathname_taux = fullfile(current_path, 'piControlData/CMCC_CM_taux.cdf');     % CDF file from Lamont website
% pathname_coord = fullfile(current_path, 'piControlData/CMCC-CM_tauuo_Omon_piControl_r1i1p1_187001-187912.nc');                      % CDF file from Lamont website does not contain lat and lon variables. 'pathname_coord' file downloaded from ESGF website to obtain these variables.
% 
% 
...
% title('CMCC CM Surface Windstress - Original Grid', 'fontsize', 12)
% colorbar
% set(gca, 'fontsize', 12)
% 
% %% Regridding
...
%% Import Data
load piControlData/CMCC_CM/cmcc_cm_taux.mat

% %% Plot Interpolated Data for first time step
% figure(1)
% axesm miller
% surfacem(lat, lon, squeeze(taux(:,:,1)))
% title('CMCC CM Surface Windstress -  Regridded', 'fontsize', 12)
% colorbar
% set(gca, 'fontsize', 12)
